...
- **LaTeX export:** `export_to_latex(source)` or CLI `--latex` — formulas from Dedekind code as LaTeX; `print_latex(s)` displays formulas **only in the console** as Unicode (α, Δ, ∫, ½ etc.), no images in plots; future possibility: KaTeX/web (see Documentation/Console_KaTeX_Roadmap.md).
