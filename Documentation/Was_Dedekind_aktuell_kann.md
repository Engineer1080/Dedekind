...
**Stand:** Basierend auf Code und Changelogs (v1.2.2, Februar 2026). Dedekind ist ein **Prototyp** – die Sprache wird nach Python transpiliert und nutzt PyTorch/NumPy als Laufzeit.
...
- **Differenzierbare PDE:** `pde_heat_1d`, `pde_heat_2d` (Wärmeleitung); `pde_advection_1d`, `pde_advection_2d` (Advektion); `pde_wave_1d`, `pde_wave_2d` (Wellengleichung); `pde_burgers_1d`, `pde_burgers_2d` (Burgers u_t + u·∇u = ν∇²u, Upwind); Finite Differenzen + `ode_solve`.
...
- **Weitere PDE:** Wärmeleitung, Advektion, Wellengleichung und Burgers 1D/2D vorhanden; keine Maxwell-Gleichungen als Standard-API.
