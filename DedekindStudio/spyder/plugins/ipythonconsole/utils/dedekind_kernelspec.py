...
# DedekindStudio is inside the Dedekind repo. Repo root = parent of DedekindStudio.
HERE = osp.abspath(os.path.dirname(__file__))
# HERE = .../DedekindStudio/spyder/plugins/ipythonconsole/utils
# 4 dirnames -> DedekindStudio root, 5 -> repo root (Dedekind)
for _ in range(4):
    HERE = osp.dirname(HERE)
_DEDEKIND_STUDIO_ROOT = HERE
_REPO_ROOT = osp.dirname(_DEDEKIND_STUDIO_ROOT)
HERE = osp.abspath(os.path.dirname(__file__))  # restore for other uses


def get_dedekind_repo_root():
    """Return the Dedekind repo root (parent of DedekindStudio) if detectable."""
