...
Aus dem **DedekindLanguage**-Repo-Root (oder direkt in `DedekindStudio/`):
...
| Schritt | Befehl (in DedekindStudio/) |
|--------|--------------------------|
...
Nach dem ersten `pip install -e .` reicht zum Starten jeweils `python bootstrap.py` (aus `DedekindStudio/`). Für Dedekind Studio: Nach euren Änderungen (Branding, Dedekind-Kernel) ebenfalls `python bootstrap.py` aus diesem Ordner ausführen.
